...
ACTIVE_PROVIDER = _PROVIDER_MAP.get(
    AI_PROVIDER.lower().strip(), AI_PROVIDER.lower().strip()
)
...
class ProjectScanner:
    ...
    def _purpose(self, fn, content, ext, ai, sc):
        ...
        if ai:
            ...
            if any(k in ai for k in ['embedding','vector','chromadb',
                                      'pinecone','faiss']):
                return "Vector Store"
            if 'rag' in ai:
                return "RAG Pipeline"
            if any(k in ai for k in ['prompt','completion','chat_completion']):
                return "LLM Interaction"
            if any(k in ai for k in ['model','predict','inference']):
                return "ML Inference"
            if any(k in ai for k in ['transformers','torch','tensorflow','keras']):
    ...
    def _desc(self, fn, content, ext, ai, sc, is_ai, is_sc):
        ...
        imps = re.findall(r'^(?:import|from)\s+(\S+)', content, re.MULTILINE)
        if imps:
    ...
    def _ftype(self, ext, ai, sc):
        ...
        return {
            ...
            '.sh':'Shell',  '.toml':'TOML',   '.ipynb':'Notebook',
        }.get(ext, 'Other')
...
# ============================================================
# HTML BUILDER — shared CSS/nav used by app.py
# ============================================================
class HTMLBuilder:
    """
    All HTML rendering lives here.
    app.py calls these static methods and serves the result.
    """
...
.nav-btn{padding:8px 18px;border-radius:8px;font-size:12px;font-weight:600;
  text-decoration:none;background:#111827;color:#94a3b8;border:1px solid #1e293b;
...
.hcards{display:flex;gap:24px;justify-content:center;flex-wrap:wrap;margin-top:32px}
...
.hcard:hover{transform:translateY(-4px);box-shadow:0 8px 24px rgba(0,0,0,.4)}
...
<div class="topbar">
  ...
  <div style="display:flex;align-items:center;gap:16px">
    <span style="font-size:12px;color:#818cf8;font-weight:600">
      🧠 {cfg['name']} &nbsp;•&nbsp; ⚡ {cfg['provider'].upper()}</span>
...
<div class="footer">
  ...
  {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    ...
    def page(title, active, body, refresh=0):
        """Wrap body in full HTML page with nav."""
...
<div class="urow">
  <div style="flex:1">
    ...
    <div class="ubar">
      <div class="ufill" style="width:{p}%;background:{c}"></div></div>
  </div>
  <div><div class="uval">{used:,}</div>
       <div class="ulim">/ {limit:,}</div></div>
</div>"""
    ...
    # ═══════════════════════════════════════════════════════════
    # PAGE BUILDERS — called from app.py routes
    # ═══════════════════════════════════════════════════════════
...
<div class="hero">
  ...
  <p>Real-time monitoring for AI agents, scripts, tokens, requests
...
<div class="container">
  ...
  <div class="grid g6">
    <div class="card blue"><div class="lbl">Total Files</div>
      <div class="val">{len(files)}</div></div>
    <div class="card purple"><div class="lbl">AI Agents</div>
      <div class="val">{ai}</div></div>
    <div class="card green"><div class="lbl">Scripts</div>
      <div class="val">{sc}</div></div>
    <div class="card orange"><div class="lbl">Main Files</div>
      <div class="val">{mn}</div></div>
    <div class="card cyan"><div class="lbl">Tokens/Day</div>
      <div class="val">{td:,}</div></div>
    <div class="card red"><div class="lbl">Requests/Day</div>
  ...
  <div class="grid g3">
    <div class="card dark"><div class="lbl">🔲 CPU</div>
      {HTMLBuilder._pbar(sys_m.get('cpu_pct',0),
                         HTMLBuilder._bc(sys_m.get('cpu_pct',0)))}
      <div class="pbar-detail">
        <span>Processor</span>
        <span>{sys_m.get('cpu_pct',0):.1f}%</span></div></div>
    <div class="card dark"><div class="lbl">🧠 Memory</div>
      {HTMLBuilder._pbar(sys_m.get('mem_pct',0),
                         HTMLBuilder._bc(sys_m.get('mem_pct',0)))}
      <div class="pbar-detail">
        <span>{sys_m.get('mem_used',0)} GB</span>
        <span>/{sys_m.get('mem_total',0)} GB</span></div></div>
    <div class="card dark"><div class="lbl">💾 Disk</div>
      {HTMLBuilder._pbar(sys_m.get('disk_pct',0),
                         HTMLBuilder._bc(sys_m.get('disk_pct',0)))}
      <div class="pbar-detail">
        <span>{sys_m.get('disk_used',0)} GB</span>
        <span>/{sys_m.get('disk_total',0)} GB</span></div></div>
  </div>
...
<div class="container">
  ...
  <div class="tw"><div class="ts"><table>
    <thead><tr><th>Type</th><th>Name</th><th>Path</th><th>Purpose</th>
      <th>Description</th><th>Size</th><th>Flags</th></tr></thead>
    <tbody>{rows}</tbody>
  </table></div></div>
</div>"""
    ...
    def model():
        ...
        for prov, pcfg in MODEL_REGISTRY.items():
            for mname, mcfg in pcfg['models'].items():
                active = (prov == ACTIVE_PROVIDER and mname == ACTIVE_MODEL)
                hl  = "color:#22c55e;font-weight:700" if active else "color:#475569"
...
<div class="container">
  ...
  <div class="grid g3">
    <div class="card blue"><div class="lbl">Tokens / Min</div>
      <div class="val">{cfg['tpm']:,}</div></div>
    <div class="card purple"><div class="lbl">Tokens / Hour</div>
      <div class="val">{cfg['tph']:,}</div></div>
    <div class="card cyan"><div class="lbl">Tokens / Day</div>
      <div class="val">{cfg['tpd']:,}</div></div>
    <div class="card green"><div class="lbl">Requests / Min</div>
      <div class="val">{cfg['rpm']:,}</div></div>
    <div class="card orange"><div class="lbl">Requests / Hour</div>
  ...
  <div class="grid g3">
    <div class="card dark"><div class="mi-l">Input / 1K tokens</div>
      <div style="font-size:22px;font-weight:700;color:#22c55e;margin-top:8px">
        ${cfg['cost_in']:.5f}</div></div>
    <div class="card dark"><div class="mi-l">Output / 1K tokens</div>
      <div style="font-size:22px;font-weight:700;color:#f59e0b;margin-top:8px">
        ${cfg['cost_out']:.5f}</div></div>
    <div class="card dark"><div class="mi-l">Env Vars</div>
  ...
  <div class="tw"><div class="ts"><table>
    <thead><tr><th>Provider</th><th>Model</th><th>TPM</th><th>RPM</th>
      <th>Context</th><th>In $/1K</th><th>Out $/1K</th><th>Status</th></tr></thead>
    <tbody>{rows}</tbody>
  </table></div></div>
</div>"""
...
<div class="container">
  ...
  <div class="tw"><div class="ts"><table>
    <thead><tr><th>Timestamp</th><th>Path</th><th>Total</th>
      <th>AI</th><th>Scripts</th><th>Main</th></tr></thead>
    <tbody>{rows}</tbody>
  </table></div></div>
</div>"""
...
<div class="container">
  ...
  <div style="text-align:center;margin-top:32px">
    <a href="/" class="nav-btn" style="font-size:15px;padding:12px 32px">
      🏠 Home</a>
