...
def find_py_files(root):
    excluded = {
        ...
        ".pytest_cache", "node_modules", "dist", "build", ".arena",
    }
    files = []
    ...
    for fp in find_py_files(root):
        try:
            ...
            issues.append({
                ...
                "WHY": "Syntax invalid - deploy will crash on import",
                "SOLUTION": f"Fix syntax in {fp}:{line}",
            })
            ...
            issues.append({
                ...
                "WHY": e.msg,
                "SOLUTION": f"Fix line {e.lineno} in {fp}",
            })
        except Exception:
            pass
    return issues
...
def check_requirements(root):
    ...
    req_files = (
        ...
        glob.glob(os.path.join(root, "**/requirements*.txt"), recursive=True)
    )
            ...
            for idx, line in enumerate(lines, 1):
                ...
                if " " in line and not any(x in line for x in ["#", ";", "==", ">=", "<=", "~="]):
                    if not re.match(r"^[a-zA-Z0-9_.-]+", line):
                        issues.append({
                            "FILE": rf, "LINE": str(idx),
                            "PRESENT_ERROR": f"Invalid requirement: {line}",
                            "EXPECTED_VALUE": "Valid pip package e.g. flask==2.3.0",
                            "WHY": "pip install will fail",
                            "SOLUTION": f"Fix line {idx} in {rf}",
                        })
        except Exception:
            pass
    return issues
...
def check_docker(root):
    issues = []
    dockerfiles = (
        glob.glob(os.path.join(root, "**/Dockerfile"), recursive=True) +
        glob.glob(os.path.join(root, "docker/**/Dockerfile"), recursive=True)
    )
    if not dockerfiles:
        issues.append({
            "FILE": "Dockerfile", "LINE": "N/A",
            "PRESENT_ERROR": "Dockerfile not found",
            "EXPECTED_VALUE": "Dockerfile at root or docker/Dockerfile",
            "WHY": "Docker build will fail",
            "SOLUTION": "Add Dockerfile with python:3.12-slim base",
        })
    else:
            ...
            try:
                ...
                if "replace_with_ecr_image_uri" in txt or "REPLACE_WITH" in txt:
                    issues.append({
                        ...
                        "EXPECTED_VALUE": "Real image like myrepo/app:latest",
                        "WHY": "Container will fail to pull",
                        "SOLUTION": "Replace placeholder in pipeline",
                    })
            except Exception:
                pass

    for dr in [
        os.path.join(root, "Dockerrun.aws.json"),
        ...
        if os.path.exists(dr):
            try:
                ...
                if "replace_with_ecr_image_uri" in txt:
                    issues.append({
                        "FILE": dr, "LINE": "N/A",
                        "PRESENT_ERROR": "Placeholder not replaced",
                        "EXPECTED_VALUE": "Real Docker Hub image URI",
                        "WHY": "EB deploy will fail",
                        "SOLUTION": f"Replace placeholder in {dr} before deploy",
                    })
            except Exception:
                pass
    ...
    if not os.path.exists(os.path.join(root, ".dockerignore")) and dockerfiles:
        issues.append({
            "FILE": ".dockerignore", "LINE": "N/A",
            "PRESENT_ERROR": ".dockerignore missing",
            "EXPECTED_VALUE": ".dockerignore with .venv, __pycache__, .git",
            "WHY": "Image bloat and possible secret leaks",
...
def check_secrets(root):
    ...
    secret_patterns = [
        ...
        (r"AKIA[0-9A-Z]{16}",                                   "Hardcoded AWS key"),
        (r"(?i)secret\s*=\s*['\"][^'\"]{8,}['\"]",             "Hardcoded secret"),
    ]
    for fp in find_py_files(root)[:50]:
        try:
            ...
            for pat, desc in secret_patterns:
                ...
                if m:
                    ...
                    issues.append({
                        "FILE": fp, "LINE": str(line_no),
                        "PRESENT_ERROR": f"{desc}: {m.group(0)[:60]}",
                        "EXPECTED_VALUE": "Use os.getenv('KEY')",
                        "WHY": "Secrets leak to git and Docker image",
                        "SOLUTION": f"Move to env var in {fp}:{line_no}",
                    })
                    break
        except Exception:
            pass
    ...
    for fp in find_py_files(root)[:100]:
        try:
            ...
            for m in re.finditer(
                r"os\.getenv\(['\"]([A-Z_]+)['\"]|os\.environ\[['\"]([A-Z_]+)['\"]", txt
            ):
...
def try_pytest(root):
    ...
    try:
        proc = subprocess.run(
            cmd, shell=True, capture_output=True, text=True, timeout=60, cwd=root
        )
...
def format_issue(iss):
    return (
        f"FILE: {iss['FILE']}\n"
        f"LINE: {iss['LINE']}\n"
        f"PRESENT_ERROR: {iss['PRESENT_ERROR']}\n"
        f"EXPECTED_VALUE: {iss['EXPECTED_VALUE']}\n"
        f"WHY: {iss['WHY']}\n"
        f"SOLUTION: {iss['SOLUTION']}\n"
    )
...
def main():
    ...
    print(f"[test_agent] scanning: {root}")
    ...
    if (
        test_code != 0
        and "not installed" not in test_output.lower()
        and "no tests ran" not in test_output.lower()
    ):
    ...
    with open("reports/pre_deploy_report.txt", "w", encoding="utf-8") as f:
        if not all_issues:
            f.write("No issues found - ready to deploy\n")
        else:
    ...
    if not all_issues:
        ...
        if ai_available():
            summary = ask_ai(
                f"Project passed all pre-deploy checks. "
                f"Test output: {test_output[:2000]}. "
    ...
    if ai_available():
        prompt = (
            f"Project has {len(all_issues)} pre-deploy issues:\n\n"
            + "\n".join(format_issue(i) for i in all_issues[:5])
            + f"\n\nTest output:\n{test_output[:3000]}\n\n"
    ...
    _send(start, "rejected", "rejected", f"{len(all_issues)} pre-deploy issues")
    sys.exit(1)
